...
@section('header')
    <div class="flex flex-col items-center justify-center text-center">
        <h1 class="text-3xl md:text-4xl font-bold text-bone mb-2">
            <span class="text-gold">Reset</span> Password
        </h1>
        ...
        <div class="w-full max-w-md relative z-10">
            <div class="bg-ink-raised/90 backdrop-blur-sm border border-line rounded-xl shadow-lg overflow-hidden">
                <div
                    ...
                    <p class="text-sm text-bone-dim">We'll send a reset link to your email</p>
                </div>
                ...
                <form method="POST" action="{{ route('admin.password.email') }}" class="px-8 pt-8 pb-8">
                    ...
                    @if (session('status'))
                        <div class="mb-4 p-3 bg-good/10 border border-good/30 rounded-lg">
                            <p class="text-sm text-good text-center">{{ session('status') }}</p>
                        </div>
                    @endif
                ...
                <div class="px-8 pb-6">
                    <div class="bg-ink-raised2/30 border border-line/50 rounded-lg p-4">
                        <div class="flex items-start">
                            ...
                            <div>
                                <p class="text-sm text-bone-dim font-medium mb-1">Password Reset</p>
                                <p class="text-xs text-bone-dim">A secure link will be sent to your registered email
                                    address.</p>
                            </div>
                        </div>
                    </div>
