        ...
        <form action="{{ route('admin.dresses.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="bg-black/90 backdrop-blur-sm border border-gray-800 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Dress Name -->
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-400 mb-2">Dress Name <span
                    ...
                    <!-- Price -->
                    <div>
                        <label for="price" class="block text-sm font-medium text-gray-400 mb-2">Price (R) <span
                                class="text-yellow-500">*</span></label>
                        <div class="relative">
                    ...
                    <!-- Status -->
                    <div>
                        <label for="status" class="block text-sm font-medium text-gray-400 mb-2">Status <span
                                class="text-yellow-500">*</span></label>
                        <select name="status" id="status" required
                            class="w-full px-4 py-3 bg-gray-800/50 border border-gray-700 rounded-lg text-white text-sm focus:outline-none focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500/30 transition-all duration-200">
                            <option value="draft" {{ old('status') == 'draft' ? 'selected' : '' }} class="bg-gray-900">
                                Draft</option>
                            <option value="active" {{ old('status') == 'active' ? 'selected' : '' }} class="bg-gray-900">
                                Active</option>
                            <option value="out_of_stock" {{ old('status') == 'out_of_stock' ? 'selected' : '' }}
                                class="bg-gray-900">Out of Stock</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                        @enderror
            ...
            <div class="bg-black/90 backdrop-blur-sm border border-gray-800 rounded-xl p-6">
                ...
                <!-- Sizes -->
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-400 mb-3">Available Sizes <span
                            class="text-yellow-500">*</span></label>
                ...
                <!-- Colors -->
                <div>
                    <label class="block text-sm font-medium text-gray-400 mb-3">Available Colors <span
                            class="text-yellow-500">*</span></label>
                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                        @foreach ($availableColors as $value => $label)
                            <label
                                ...
                                @else
                                    <div
                                        class="ml-auto w-4 h-4 rounded-full bg-gradient-to-r from-red-400 via-blue-400 to-yellow-400 border border-white/20">
                                    </div>
                                @endif
                            </label>
                        @endforeach
            ...
            <div class="bg-black/90 backdrop-blur-sm border border-gray-800 rounded-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Turnaround Time -->
                    <div>
                        <label for="turnaround_time" class="block text-sm font-medium text-gray-400 mb-2">Turnaround Time
                            <span class="text-yellow-500">*</span></label>
                    ...
                    <!-- Expected Delivery -->
                    <div>
                        <label for="expected_delivery" class="block text-sm font-medium text-gray-400 mb-2">Expected
                            Delivery <span class="text-yellow-500">*</span></label>
                ...
                <div class="mt-6">
                    <label class="flex items-center cursor-pointer group">
                        <input type="checkbox" name="is_featured" value="1"
                            ...
                            class="h-4 w-4 text-yellow-500 bg-gray-800 border-gray-600 rounded focus:ring-yellow-500/30 focus:ring-offset-0">
                        <span class="ml-2 text-sm text-gray-300 group-hover:text-yellow-500 transition-colors">Mark as
                            featured dress</span>
                    </label>
                </div>
            </div>
            ...
            <div class="bg-black/90 backdrop-blur-sm border border-gray-800 rounded-xl p-6">
                ...
                <!-- Main Image -->
                <div class="mb-6">
                    <label for="main_image" class="block text-sm font-medium text-gray-400 mb-2">Main Image <span
                            class="text-yellow-500">*</span></label>
                    <div
                        class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-700 border-dashed rounded-lg bg-gray-800/20">
                        <div class="space-y-2 text-center">
                            ...
                            <div class="flex text-sm text-gray-400">
                                <label for="main_image"
                                    class="relative cursor-pointer bg-gray-800/50 rounded-md font-medium text-yellow-500 hover:text-yellow-400 focus-within:outline-none px-3 py-2 border border-gray-700 hover:border-yellow-500/50 transition-colors">
                ...
                <!-- Gallery Images -->
                <div>
                    <label for="gallery_images" class="block text-sm font-medium text-gray-400 mb-2">Gallery Images <span
